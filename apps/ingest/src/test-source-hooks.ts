...
/**
 * Lets a child `node` process run ingest from its TypeScript sources, the way vitest runs them, for
 * the test that needs a process of its own (`main.test.ts`). Loaded with `--import`. A copy of the
 * emulator's hooks: one app never imports another's sources.
 *
 ...
 * shared package declares a parameter property (`ConfigError`), which plain
