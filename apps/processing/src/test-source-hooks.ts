...
/**
 * Lets a child `node` process run processing from its TypeScript sources, the way vitest runs
 * them, for the test that needs a process of its own (`main.test.ts`). Loaded with `--import`. A
 * copy of ingest's hooks: one app never imports another's sources.
 *
 ...
 * shared package declares parameter properties (`ConfigError`), which plain type stripping rejects.
 *
