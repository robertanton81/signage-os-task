...
**Amendment of 2026-09-15 (decision 10, the health check's user):** the probe runs as the `rabbitmq` user through `gosu`. `docker exec` runs as root in this image, and a probe that ran before the server had written `/var/lib/rabbitmq/.erlang.cookie` created it as root, mode 0400; the server, running as `rabbitmq`, then failed in `auth:init` with `eacces` and exited 1 — the CLI guide's "shared secret seeding race condition". It hit the CI runner in three of four runs with `start_interval: 1s`, invisible until the test setup printed the container's log (integration-tests spec, the global setup's amendment of the same day). Reproduced with the real server: a root probe run after the entrypoint and before the server's boot leaves a root-owned cookie and the server exits 1 with `eacces`; the same probe as `rabbitmq` leaves it healthy. The probe as the server's own user makes the order irrelevant: whoever writes the cookie, the other can read it. The entrypoint chowns a cookie that exists before it runs, so only a probe inside the boot window triggers the race, which is why a fast machine never saw it. Pre-existing since this file was written; the processing refactor of 2026-09-15 touched neither Compose file.
...
services:
  rabbitmq:
    ...
    healthcheck:
      # Stage-4 node health check: opens a TCP connection to every enabled listener. As the
      # `rabbitmq` user (amendment of 2026-09-15, decision 10).
