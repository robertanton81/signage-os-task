...
| 2   | Package manager                   | pnpm 10.29.2 pinned via `packageManager`; a workspace **catalog** holds every dev-dependency version                                                                                                                                                                       | The assignment requires pnpm. `catalog:` keeps exactly one version of each tool across packages and makes upgrades a one-line change. Today only the root manifest consumes it; it is groundwork for the runtime dependencies of step 2 (AMQP client, MongoDB driver, validation) that `apps/ingest` and `apps/processing` will share.                                                              |
...
- [Vitest 4.1.11 test projects](https://github.com/vitest-dev/vitest/blob/v4.1.11/docs/guide/projects.md) (docs at the release tag) — `test.projects` ("`workspace` is deprecated since 3.2 and replaced with the `projects` configuration"), inline configs, `extends: true` to inherit root options, `--project <name>` filtering, project names must be unique, `coverage`/`reporters` only at the root.
- [Vitest 4.1.11 CLI](https://github.com/vitest-dev/vitest/blob/v4.1.11/docs/guide/cli.md) — a positional argument "will run only the test file that contains `<arg>` in their paths" (substring, no glob); `-r, --root <path>` is listed in the generated option table ([cli-generated.md](https://github.com/vitest-dev/vitest/blob/v4.1.11/docs/guide/cli-generated.md)), as are `passWithNoTests`, `fileParallelism`, `testTimeout` and `hookTimeout`.
- Local probe (2026-09-08, vitest 4.1.11 with the root config): `pnpm test` runs both projects; `pnpm --filter @telemetry/ingest test` (`vitest run --root ../.. apps/ingest` from the package directory) picked up the root config and executed only the one test file under `apps/ingest`; `pnpm test:integration` exits 0 with no files because of `passWithNoTests`. A throwaway `apps/ingest/src/wiring.test.ts` importing `@telemetry/shared` passed through the alias, `tsc -b` resolved the shared types through `exports` → `dist/index.d.ts`, and `node` executed the built `apps/ingest/dist` output through the workspace symlink; the files were removed afterwards.
...
- `apps/*`: `src/main.ts` declares the service name only; the real entry points are wired in steps 3–5. Each app already declares `@telemetry/shared` as a `workspace:*` dependency and a TypeScript project reference, so `tsc -b` builds `packages/shared` first and the workspace link exists from day one. Nothing imports the package yet: real cross-package imports, and with them continuous coverage of the Vitest alias, start in steps 3–5. The alias and the `exports` → `dist` path were verified once with a throwaway test (see Research) rather than kept alive by a contrived placeholder import.
- Per-package scripts: `build` and `typecheck` (both `tsc -b`), `lint` (`eslint .`), `test` (`vitest run --root ../.. <package dir>`: the root config is loaded from `--root` and the positional argument narrows the run to that package's files), and for apps `start` (`node dist/main.js`, the future Docker `CMD`), so `pnpm --filter @telemetry/<pkg> <script>` works as `CLAUDE.md` documents. Packages declare no dev-dependencies of their own: `tsc`, `eslint` and `vitest` resolve through Node's ancestor `node_modules` lookup and pnpm's PATH augmentation with ancestor `node_modules/.bin`.
...
`defineConfig` with `globalIgnores(['**/dist/**', '**/node_modules/**', '**/coverage/**'])`; for `**/*.ts`: `js.configs.recommended`, `tseslint.configs.recommendedTypeChecked`, `languageOptions.parserOptions = { projectService: true, tsconfigRootDir: import.meta.dirname }`, rules `no-console: error`, `@typescript-eslint/no-non-null-assertion: error`, `@typescript-eslint/consistent-type-imports: error`, `@typescript-eslint/switch-exhaustiveness-check: error`, `@typescript-eslint/no-unused-vars` with `_`-prefixed exceptions; root TypeScript config files (`*.config.ts`, i.e. `vitest.config.ts`) get `tseslint.configs.disableTypeChecked`; `.js` files (`eslint.config.js`) are linted with `js.configs.recommended` only and never reach the type-aware block; `eslint-config-prettier/flat` last.
...
### One flat root tsconfig without project references

A single `tsconfig.json` with `include: ["apps/*/src", "packages/*/src"]` and `tsc --noEmit` would make `typecheck` emit nothing and avoid the TS6310 limitation. It was rejected because each app needs its own `dist/` for its Docker image, `packages/shared` needs declaration output so the apps consume it through `exports` like any dependency, and references give incremental per-package builds. The cost is that `typecheck` writes to gitignored `dist/`, which is harmless.
