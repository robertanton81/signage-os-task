> **STATUS: SHIPPED 2026-09-15.** Landed as 6 commits ending at `f5b4fc6`, this header and the ledger line in the commit after it: `4ac1593` (the plan), `701ef1a` (Task 1, C11b), `3b7356f` (Task 2, the specs, the ledger and the consumer's comments), `98faa83` (Task 3), `8263895` (Task 4), `f5b4fc6` (review correction, see below). CI: push run `34976955158` of `f5b4fc6355a3a9c052543813ab71a52596bf6d85`, `success`. The unchecked `- [ ]` boxes below are historical — work is done. **Do not re-execute this plan.** If you're modifying C11b or the consumer's shutdown, work directly in `test/integration/processing-consumer.test.ts` and `apps/processing/src/consumer.ts`.
>
> **Plan-vs-reality corrections discovered during execution:**
>
> **Library/version drift:** none — no dependency changed (RabbitMQ 4.3.5, amqplib 2.0.1, vitest 4.1.11 as pinned).
>
> **Plan code prescriptions that needed adjustment:** (1) The plan file itself was not in any commit, so `git status` could not be clean before the first task commit; it was committed first as `4ac1593` (`Add the CI follow-up implementation plan`). (2) C11b's title still said "the next instance gets exactly the rest"; renamed to "… gets the rest and at most their redeliveries as duplicates" (the plan did not mention the title; the historical copy in the integration-tests plan keeps the old one). (3) Mutant M2's failure detail differed from the prediction: `a` read `acked 0, failed 48, abandoned 2` (handlers aborted while inside the store call end as permanent rejects), not `abandoned 50`; the failing assertion was the `sa` one as predicted, and `b.received === 201 = 151 + 50` satisfied the `b` assertion alone, as Edit 2.11 anticipated. (4) Prettier left the test file unchanged and re-padded the three spec tables (the 68- and 98-line spec diffs of `3b7356f` are column padding). (5) M1's threshold 34 drops exactly the 35th–50th frames because the consumer increments `acked` before it sends the frame — confirmed, `acked 166 of 150` reproduced verbatim.
>
> **Corrections applied during review (commit `f5b4fc6`):** test-quality-reviewer BLOCKING on Task 1 — the `for` loop over `redelivered()` runs zero times on a fast machine (n = 0 here, 16 on the runner), so the `stale` + `duplicate` classification of a redelivered message had no deterministic test in the repository. Fixed with the handler unit test `absorbs a redelivered message the stopped instance had stored and applied as a stale duplicate` (`apps/processing/src/handler.test.ts`: insertEvent → duplicate, applyState before at the message's own key, `redelivered: true` → result and log line `{ outcome: 'stale', duplicate: true }`); red-checked with a scratch mutant `duplicate ? 'created' : classifyOutcome(...)` that failed exactly that test of the file's 30. A harness fault forcing the broker to redeliver was rejected: it needs the consumer's own channel (product code, A1). Criterion 13's file list therefore has a fourth entry, `apps/processing/src/handler.test.ts` (test-only; no product behaviour changed). Second review round: PASS.
>
> **Deferrals worth tracking:** (1) `redelivered()` is computed twice in C11b (for `n` and for the loop) — harmless; reuse one array when C11b is next touched (code-reviewer suggestion). (2) Assert `b.stats().duplicate === n` next to the `sb` assertion as a second independent signal (test-quality suggestion). (3) Add half a sentence to the handshake test's comment on why the set-based check was chosen over stopping the connection at the first `backoff` — A6: stopping inside the poll callback couples the test to the poll cadence (test-quality suggestion). (4) The C11b loop stays a CI-only cross-check; the deterministic claim lives in the handler unit test and in this record of the mutant runs. For the README's known limits: T70 is a protocol limit (AMQP 0-9-1 confirms no acknowledgement), bounded to the deliveries of one drain and absorbed by the dedup.
>
> **Review record:** Stage 1 PASS (Task 1, Task 2); Stage 2 code-reviewer PASS on Task 1 (two suggestions); test-quality on Task 1 REVISE → PASS after `f5b4fc6`; test-quality on Tasks 3–4 PASS (one suggestion). Tasks 3 and 4 skipped Stage 1 and the code review (under 30 lines each, an assertion tolerance only); Task 2 skipped Stage 2 (documentation and comments only).
>
...
| #   | Decision                                                                                                                                                                                                                                                                                                                                                                                                             | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | The consumer keeps the channel-first close of `547b310`; no behaviour change in product code. Only the two doc comments in `consumer.ts` that repeat the disproved claim (`stop()`, `#closeLink`) are corrected, in Task 2 (plan review, 2026-09-15).                                                                                                                                                                | The user chose option A on 2026-09-15 ("as you recommend"). The close order removes the losses a fast machine sees (7 of 7, then 8 starved-container runs) and costs no time; only the claim attached to it was wrong, and the code's own comments carry that claim too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| A3  | The at-least-once assertions are the pre-`f1792c9` shape: `b.received === 151 + n`, `b.acked === 151 + n`, `n <= 50`, every redelivered line `outcome: 'stale', duplicate: true`.                                                                                                                                                                                                                                    | That shape was measured on this machine before the fix (one redelivery) and matches CI's numbers (16); `stale` because the stopped instance applied the state before it acknowledged, `duplicate` because the event insert hits the unique index. The `+1` is the barrier.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- `gh run list --workflow ci.yml --event push --branch main --json databaseId,headSha,status,conclusion` — measured 2026-09-15: `34950460095 5745438db2177a45f9625c7477be491effc5afbd completed failure` and the run of `9e99d03` below it; `headSha` is the full 40-character sha. Task 5.
...
```text
and C11b asserts the exact remainder again (`547b310`, `f1792c9`).
```

New:

```text
and C11b asserted the exact remainder again (`547b310`, `f1792c9`). **Corrected 2026-09-15 after `/verify`:** the exact remainder held on this machine only; the CI runner redelivered 16 of the 50 in both runs, because the quorum-queue client stashes settles above 32 pending commands and never flushes the stash at a channel close — see decision 20 and T70 (reopened). C11b asserts `151 + n` (the other 150, its barrier message and the `n` redeliveries) with every redelivered delivery `duplicate` and `stale`.
```
...
```text
Amended 2026-09-15: the link close is the channel's close-ok, then the connection, within one close budget (T70, `547b310`).
```

New:

```text
Amended 2026-09-15: the link close is the channel's close-ok, then the connection, within one close budget (T70, `547b310`), which narrows but cannot close the window of a lost acknowledgement (T70 reopened after `/verify` the same day; the close-ok does not prove that the queue applied the acknowledgements).
```
...
```text
`b.received === 150` and no `delivery processed` line of `b` carries `redelivered: true` (the 150 were never delivered before; amended to `150 + n` on 2026-09-15 after the plan's probe measured the lost last acknowledgement, and back to the exact count the same day once the consumer closed the channel before the connection, `547b310`, T70); 200 distinct events, each once, and 20 alerts;
```

New:

```text
`b.received === 151 + n` and `b.acked === 151 + n`, where `n` is the number of `delivery processed` lines of `b` with `redelivered: true`, each `outcome: 'stale'` and `duplicate: true`, `n <= 50` (the 150 were never delivered before, the 151st is the barrier; `n` was 0 on this machine after `547b310` and 16 on the CI runner — T70: an acknowledgement sent right before the channel close is not always applied by the queue); 201 distinct events (the load's 200 and the barrier), each once, and 20 alerts;
```
...
```text
(1) T70 — closed the same day, after this header, by `547b310` (the channel's close-ok before the connection close, one shared close budget) and `f1792c9` (C11b asserts the exact remainder again); the `150 + n` tolerance the review notes above describe is history.
```

New:

```text
(1) T70 — closed the same day, after this header, by `547b310` (the channel's close-ok before the connection close, one shared close budget) and `f1792c9` (C11b asserted the exact remainder again), then reopened on 2026-09-15 after `/verify`: the CI runner redelivered 16 of the 50, the `150 + n` form is back (as `151 + n`, behind a consumer-count wait and a barrier message) with the mechanism recorded in the consistency spec (`docs/plans/2026-09-15-ci-followup-plan.md`).
```
...
1. Read this plan.
2. `git log --oneline 5745438..HEAD` — the subjects above name the task each commit completes; `git status` shows a task in progress.
