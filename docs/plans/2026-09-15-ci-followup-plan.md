> **STATUS: SHIPPED 2026-09-15.** Landed as 6 commits ending at `6231193`, this header and the ledger line in the commit after it: `3eb43bd` (the plan), `9a02e62` (Task 1, C11b), `46e1c19` (Task 2, the specs, the ledger and the consumer's comments), `0d3e6f1` (Task 3), `2958fa7` (Task 4), `6231193` (review correction, see below). CI: push run `34976955158` of `6231193feb075f026166dd83b3a4d046d5c532fd`, `success`. The unchecked `- [ ]` boxes below are historical — work is done. **Do not re-execute this plan.** If you're modifying C11b or the consumer's shutdown, work directly in `test/integration/processing-consumer.test.ts` and `apps/processing/src/consumer.ts`.
>
...
>
> **Review record:** Stage 1 PASS (Task 1, Task 2); Stage 2 code-reviewer PASS on Task 1 (two suggestions); test-quality on Task 1 REVISE → PASS after `6231193`; test-quality on Tasks 3–4 PASS (one suggestion). Tasks 3 and 4 skipped Stage 1 and the code review (under 30 lines each, an assertion tolerance only); Task 2 skipped Stage 2 (documentation and comments only).
>
