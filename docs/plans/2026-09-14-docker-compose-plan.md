> **STATUS: SHIPPED 2026-09-14.** Landed as 9 commits, `48cf782..affdbd5`: the plan commit, one commit per task (two for Task 4: the script and its run record), the review fix `9a82390`, the run-record update `464aac5` and the ledger (`affdbd5`). The full pre-flight passed right before the last commit: `pnpm format:check && pnpm lint && pnpm typecheck && pnpm test`, 874 tests in 41 files (855 unchanged, 19 new in `scripts/compose-check-lib.test.mjs`). `/verify` ran on 2026-09-14 at `9903d3f`: all 18 criteria passed — the script runs repeated live (scaled: `consumers=3`, `split=[5, 5]`, `device_state=10` across three replicas; `--down`: no container or volume left; 25 devices: `device_state=25`; `--sacle` rejected before any Docker call) — and so did the standing assignment criteria on the live scaled stack: two invalid frames rejected at `warn` (`invalid_json`, `invalid_schema`) with the connection kept open and both ingest replicas healthy; three processing replicas with 287/288/288 deliveries creating exactly ten state documents, `failed=0`; `/readyz` 503 and `unhealthy` on every ingest and processing replica while RabbitMQ was stopped, `ready` and `healthy` 40 s after it returned, still `failed=0`; `docker compose stop processing` returning within a second with `shutting down` → `consumer stopping` → `stopped` in 13 ms and exit 0 on all three, the queue holding 542 messages after the restart and empty with three consumers 25 s later, `telemetry.dead` empty; `EMULATOR_CHAOS=duplicate,out-of-order` counted as `duplicate=26 stale=44 failed=0` with the ten documents unchanged; `EMULATOR_EVENT_INTERVAL_MS=250` raising the rate from 12.6 to 48.6 messages/s. The unchecked `- [ ]` boxes below are historical — work is done. **Do not re-execute this plan.** If you're modifying the development stack, work directly in `Dockerfile`, `.dockerignore`, `docker-compose.yml`, `scripts/compose-check.mjs` and `scripts/compose-check-lib.mjs`.
>
...
**Branch:** `worktree-compose-dev-stack` (this worktree; the spec is its first commit). Small atomic commits, imperative subjects, no `Co-Authored-By`, no AI mention. After `/verify`, `main` is fast-forwarded to the branch tip, as the WebSocket transport branch was (`283ec49`); that merge is not one of the tasks.
...
| #   | Assumption                                                                                                                                                                                                                                                                                                                                                                          | Basis                                                                                                                                                                                                                                                             |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | The work lands on `worktree-compose-dev-stack`, the branch that already carries the spec, and reaches `main` by fast-forward after `/verify`.                                                                                                                                                                                                                                       | The transport precedent (`283ec49`); the standing rule against parallel sessions on `main`.                                                                                                                                                                       |
| A2  | The plan is committed by the `/plan` session as its own commit on the branch (the spec's precedent, `4031b8c`); Task 1 then carries only the two `TODO.md` forward notes. The spec amends no earlier spec, so nothing else is applied.                                                                                                                                              | The processing precedent (Task 1 of `docs/plans/2026-09-13-processing-plan.md` added the step 7 item); the compose spec's "Forward note for step 7" and "Requirements the README must state"; a worktree can be deleted with its session, a pushed commit cannot. |
...
| A15 | Rows T55–T60 are appended to the consistency spec in that list's own column names (`Compromise`, `What is given up`, `When it starts to matter`, `Upgrade path`, `Decision`), introduced by one sentence in the style of the T44–T49 block, with the Decision column pointing at `compose spec, N`.                                                                                 | The T44–T49 precedent (`2699903`); the compose spec's table already follows the shape.                                                                                                                                                                            |
...
| A17 | The test count stays at 855 in 40 files (`pnpm test` at `c2f3669`); any other number is a regression to investigate, not to accept.                                                                                                                                                                                                                                                 | This plan adds no test and touches no source.                                                                                                                                                                                                                     |
...
| 12  | Invariants 1, 2, 3 and 5 are untouched: no file under `apps/` or `packages/` changes                                                           | `git diff --stat 4031b8c..HEAD -- apps packages` is empty at the end of Task 5                                                                                                                                                 |
...
1. Read this plan.
2. `git log --oneline 4031b8c..HEAD` — the first commit after the spec is this plan; then each task ends with exactly one commit whose subject is quoted in the task (Task 4 ends with two, plus any fix commits before them).
