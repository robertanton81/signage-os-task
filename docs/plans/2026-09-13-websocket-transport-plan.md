...
> - Task 5, scenario 25 (added in `beec56f` after the code review): a valid message that arrives in the same read as a binary one is dropped once the connection has decided to close; produced deterministically by sending both while the connection is paused.
