> **STATUS: SHIPPED 2026-09-14.** Landed as 41 commits, `5cdd8aa..8195838`: the plan commit, one or more commits per task, the fixes the reviews asked for, the scripted-run evidence (`2001046`) and the ledger (`8195838`). The two merge commits of the WebSocket transport branch inside the range (`226ff9a`, `2a92803`) are not counted; `3a3cf3a`, the shared health server tests committed with a `test:` prefix before Task 2's move, is. The full pre-flight passed right before the last commit: `pnpm format:check && pnpm lint && pnpm typecheck && pnpm test`, 855 tests in 40 files, 288 of them in `apps/processing`. `/verify` ran on 2026-09-14 at `8195838`: all 19 criteria and the 6 standing assignment criteria passed (`.local/reviews/2026-09-14-processing-verify.md`). The unchecked `- [ ]` boxes below are historical; the work is done. **Do not re-execute this plan.** If you are changing processing, work directly in `apps/processing/src/`.
>
> **Process notes:** the session that ran Tasks 1–12 was interrupted after Task 12's review fixes, with one test edit uncommitted (committed on `main` as `3e43416` while the branch below was being finished) and the Task 13 script drafted but never run. A fresh session finished Tasks 13 and 14 in a git worktree on the branch `processing-task-13`, rebased onto `3e43416` before the fast-forward, so the review findings of Tasks 1–12 are known only from the commit messages, summarised below. Nothing the scripted run found was in the code: both defects of the first run were the script's own.
>
...
>
> **Plan code prescriptions that needed adjustment:** assumptions A2, A5 and A7 held as written (a `StoreError` class, `'ready' | 'aborted'` from `start`, `maxTimeMS` inside the ping command). The consumer state gained a `registering` status that Task 7's table did not have (`57ede57`, the spec amended in `3513b0d`): `open/idle` had meant both "no registration" and "a consume in flight", so a store outage reported during a pending consume could leave the consumer registered on a dead store, or a second `store_ready` could issue a second consume on one channel. The handler tracks its own settled promise in the registration, so the consume callback dispatches it with the plain `void` call the plan names for invariant 4 (`e9ca0ac`).
>
> **Corrections applied during review:** `cd54e41` (three configuration tests that could not fail), `f93df6f` (the size check proven to run before the UTF-8 decode, the exact `MAX_FRAME_BYTES` bound), `43823ea` (a server failure with an unrelated label), `942f460` (the duplicate-key rule phrased as the store contract), `09a6614` (the smallest sequence gap pinned; the watermark fields spread after the payload so invariant 1 holds by construction), `9b914be` (the watermark wins a colliding payload key), `0278de1` (`isIndexConflict` named and tested), `a4bb4c1` and `17694d2` (an aborted store start logs no further attempt), `2d74b09` (the shared `DUPLICATE_KEY_ERROR_CODE` in the handler's collision view), `b6968e5` (the redelivered flag, an existing alert and the abort checkpoints tested), `ffdac47` (an aborted start resolves even when its attempt ended in an index conflict), `3e43416` (a positive reconnect delay asserted instead of a typed `expect.any`).
>
