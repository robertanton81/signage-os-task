> **STATUS: SHIPPED 2026-09-14.** Landed as 41 commits, `4673305..2699903`: the plan commit, one or more commits per task, the fixes the reviews asked for, the scripted-run evidence (`44f4e33`) and the ledger (`2699903`). The two merge commits of the WebSocket transport branch inside the range (`7fea3a8`, `283ec49`) are not counted; `f257340`, the shared health server tests committed with a `test:` prefix before Task 2's move, is. The full pre-flight passed right before the last commit: `pnpm format:check && pnpm lint && pnpm typecheck && pnpm test`, 855 tests in 40 files, 288 of them in `apps/processing`. `/verify` ran on 2026-09-14 at `2699903`: all 19 criteria and the 6 standing assignment criteria passed (`.local/reviews/2026-09-14-processing-verify.md`). The unchecked `- [ ]` boxes below are historical; the work is done. **Do not re-execute this plan.** If you are changing processing, work directly in `apps/processing/src/`.
>
> **Process notes:** the session that ran Tasks 1–12 was interrupted after Task 12's review fixes, with one test edit uncommitted (committed on `main` as `036a34a` while the branch below was being finished) and the Task 13 script drafted but never run. A fresh session finished Tasks 13 and 14 in a git worktree on the branch `processing-task-13`, rebased onto `036a34a` before the fast-forward, so the review findings of Tasks 1–12 are known only from the commit messages, summarised below. Nothing the scripted run found was in the code: both defects of the first run were the script's own.
>
> **Library and version drift:** none. mongodb 7.6.0, amqplib 2.0.1, zod 4.6.2, vitest 4.1.11 and Node 24.21.0 are as pinned. Two facts from the broker and the driver shaped the run, not the code: the management HTTP API's queue counts lag by `collect_statistics_interval` (5 s by default), and a paused `mongod` executes the writes it had already received once it is unpaused, so the client's timeout turns into an at-least-once duplicate on redelivery (scenario 9 below).
>
> **Plan code prescriptions that needed adjustment:** assumptions A2, A5 and A7 held as written (a `StoreError` class, `'ready' | 'aborted'` from `start`, `maxTimeMS` inside the ping command). The consumer state gained a `registering` status that Task 7's table did not have (`bc255ef`, the spec amended in `0bd7f0b`): `open/idle` had meant both "no registration" and "a consume in flight", so a store outage reported during a pending consume could leave the consumer registered on a dead store, or a second `store_ready` could issue a second consume on one channel. The handler tracks its own settled promise in the registration, so the consume callback dispatches it with the plain `void` call the plan names for invariant 4 (`801b2a9`).
>
> **Corrections applied during review:** `b66d116` (three configuration tests that could not fail), `73fbc41` (the size check proven to run before the UTF-8 decode, the exact `MAX_FRAME_BYTES` bound), `60da6dc` (a server failure with an unrelated label), `226734b` (the duplicate-key rule phrased as the store contract), `b50393d` (the smallest sequence gap pinned; the watermark fields spread after the payload so invariant 1 holds by construction), `65bc6bb` (the watermark wins a colliding payload key), `75268dd` (`isIndexConflict` named and tested), `70fa99e` and `666b9ec` (an aborted store start logs no further attempt), `965ef03` (the shared `DUPLICATE_KEY_ERROR_CODE` in the handler's collision view), `22c1b80` (the redelivered flag, an existing alert and the abort checkpoints tested), `80682ab` (an aborted start resolves even when its attempt ended in an index conflict), `036a34a` (a positive reconnect delay asserted instead of a typed `expect.any`).
>
> **Deferrals worth tracking:** the automated broker-and-database tests of the consumer are the step 7 item Task 1 added (T44). The `duplicate` counter counts the redeliveries a frozen database causes (scenario 9) together with ingest's exact duplicates; a README note, not a code change. `AuthenticationFailed` is retried forever (T49). The step 6 scaling check must wait for the management API's queue counts rather than read them once.
>
> **Scripted run against RabbitMQ and MongoDB (Task 13), 2026-09-14.** `node .local/research/2026-09-13-processing-scripted-run.mjs` ran the built entry point of `036a34a` as child processes (`MONGODB_TIMEOUT_MS=1000`, `AMQP_HEARTBEAT_S=2`, `PROCESSING_TRANSIENT_ATTEMPTS=3`, `SHUTDOWN_TIMEOUT_MS=3000`, `LOG_LEVEL=debug`) against throwaway `rabbitmq:4.3-management` and `mongo:8.0` containers that reported RabbitMQ 4.3.5 (Erlang 27.3.4.17) and MongoDB 8.0.30. All twelve scenarios passed; the output is in `.local/research/2026-09-13-processing-scripted-run-output.txt`. The first run of the script (`.local/research/2026-09-13-processing-scripted-run-first-output.txt`) failed scenario 8 on the script's side only: it read the queue depth once, right after the pause, and the management API's queue counts lag by the broker's statistics interval (`collect_statistics_interval`, 5 s by default), so it saw 5 where 30 had just been returned; the check now waits for the count. The same run showed scenario 11 sending the signal after all 300 messages had already been processed, so the burst is now pipelined and the signal goes out once the first delivery has been processed. No code changed between the two runs.
>
> 1. **Normal flow.** Two devices × the four types: 8 events, both `device_state` documents with the four sections and `lastEvent` at the diagnostic's `seq` 4, 2 alerts, 8 `delivery processed` lines at `debug`, `telemetry.events` empty, and `identity_unique` on `{ deviceId, sessionId, seq }` with `unique: true`.
> 2. **Duplicate.** One error diagnostic published twice: 1 event and 1 alert; the second line at `info` with `outcome: 'stale'`, `duplicate: true`, `alert: 'exists'`.
> 3. **Out of order within a section.** `metrics` 6 then 5: 2 events, the stored `metrics.seq` 6, the late line `stale`.
> 4. **Out of order across sections.** `metrics` 8 then `status` 7: outcomes `created` then `applied`; `status.seq` 7, `metrics.seq` 8, `lastEvent.seq` 8.
> 5. **Session restart and straggler.** The new session's `status` 1 and `counters` 2 replaced both sections (`counters.operationsTotal` 0); the old session's `status` 9 was `stale` and left the document unchanged.
> 6. **Two instances.** 1 045 deliveries for 1 000 distinct identities (20 devices × 50, with 5 % exact duplicates and 5 % adjacent swaps from a seeded generator): 1 000 events, every section of every device at its highest key, 120 alerts for the 120 error diagnostics, 523 deliveries processed by A and 522 by B (the broker's round robin), B stopped with exit 0.
> 7. **Poison.** `not json`, `{"type":"bogus"}`, a diagnostic with a `0xff` byte inside its message and a 70 KiB body: four `message rejected` lines (`invalid_json`, `invalid_schema`, `invalid_utf8`, `body_too_large`), `telemetry.dead` at 4 with `x-death[0]` `reason: 'rejected'`, `queue: 'telemetry.events'`, `count: 1`; `events` unchanged; the dead queue drained afterwards.
> 8. **MongoDB stopped, then back.** 30 messages published during `docker stop`: 90 `transient store failure` lines (`server_selection`, three attempts per delivery), then `consumer paused` with `returned: 30`, `/readyz` 503 `mongodb`, the queue back at 30; after `docker start`, `store ready` then `consumer registered`, all 30 stored exactly once with no `duplicate: true` line, `/readyz` 200.
> 9. **MongoDB frozen.** 10 messages after `docker pause`: the first `transient store failure` had `failure.kind` `network` (the driver's socket timeout) 1 049 ms after the publish, then `consumer paused` and 503 `mongodb`; after `docker unpause` the consumer resumed and all 10 were stored exactly once. Every one of the 10 redeliveries was processed with `duplicate: true`: the frozen server had received the inserts before the client timed out and executed them once unpaused, the at-least-once case invariant 2 absorbs.
> 10. **Broker restart.** 100 messages at 20 per second across `docker restart`: the consumer recycled with reason `channel_closed` (`delayMs` 123), then `connect_failed` four times while the broker started (`Socket closed abruptly during opening handshake`); `/readyz` answered 503 `connecting` during the restart; `consumer registered` again; all 100 stored once; no delivery was processed twice and none arrived with `redelivered: true`, because nothing was in flight when the connection closed.
> 11. **SIGTERM with deliveries in flight.** 300 messages in one pipelined burst and SIGTERM once the first had been processed: exit 0 after 18 ms with `shutting down`, `consumer stopping`, `stopped` in that order and no drain-budget line; A had processed 53, the queue held the other 247 (sum 300); a fresh instance finished them: 300 events and 30 alerts.
> 12. **Wrong credentials.** `store not ready` at attempts 0, 1 and 2 with `failure.kind` `server` and code 18 (`AuthenticationFailed`), `/readyz` 503 `mongodb`, the process alive after 5 s, exit 0 on SIGTERM; restarted with the right password: `store ready`, `/readyz` 200, exit 0.
>
> Differences from the scenario text of Task 13 below: scenario 9's first failure is `network`, not a server code 50, because the driver's socket timeout (`socketTimeoutMS` = `MONGODB_TIMEOUT_MS`) fires before the paused server can answer anything, and the 10 redeliveries were all duplicates (recorded above); scenario 10 recycles as `channel_closed`, as ingest's did, because amqplib closes the channels before it emits the connection's `close`; scenario 11 has no drain-budget line because the in-flight handlers finished within the 18 ms; the broker user comes from `RABBITMQ_DEFAULT_USER` and `RABBITMQ_DEFAULT_PASS`, because `guest` may log in only from inside the container. Over the whole run the instances logged 1 504 `delivery processed`, 56 `event already stored` (45 exact duplicates of scenario 6, 1 of scenario 2, 10 of scenario 9) and 125 `sequence gap` lines, all in the output file.
>
...
**Design spec:** `docs/specs/2026-09-13-processing-design.md` (committed in `353506a`; review-clean after two `design-reviewer` rounds, `.local/reviews/2026-09-13-processing-design-review.md`, nothing deferred to this plan). Binding above it: `docs/specs/2026-09-11-telemetry-consistency-design.md` (decisions 7–13, 16–29, the failure table) and `docs/specs/2026-09-11-shared-contract-design.md`; `docs/specs/2026-09-13-ingest-design.md` for the message ingest publishes (decisions 6 and 7) and the health server (decision 17).
...
| #   | Assumption                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Basis                                                                                                                                                                                                                                                                                                                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | This plan is copied to `docs/plans/2026-09-13-processing-plan.md` and committed as Task 1 together with the spec's amendments to the three earlier specs and `TODO.md`, which `353506a` (the spec's own commit) did not apply.                                                                                                                                                                                                                                                  | The ingest precedent (`40b3cd5`); `grep` shows none of the five amendments in the tree.                                                                                                                                                                                                                                   |
...
| A18 | The test count baseline is the number `pnpm test` prints at Task 1 (574 at `353506a`); Task 2 keeps it unchanged (moved tests, none duplicated).                                                                                                                                                                                                                                                                                                                                | The handover of 2026-09-13.                                                                                                                                                                                                                                                                                               |
...
- Expected count after Task 14: about 130 new cases on top of the 574 at `353506a`.
