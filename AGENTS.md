# Device Telemetry Processing — Project Guidance for Codex
...
- **Commit style:** imperative subject, no Conventional Commits prefix, small atomic commits (the commit history is part of the assessment). No assistant attribution in commit messages — no `Co-Authored-By` footer, no mention of Codex or any AI tool. The message describes the change, nothing else.
...
## Working rules (portable — they do not depend on anyone's personal Codex configuration)

These rules travel with the repository so that Codex behaves the same for every user who opens it. They are copied from the author's personal setup on purpose; if a personal configuration says otherwise, the rules here win inside this repository.
...
**To do** — what remains, in the order it should happen, each line naming who moves it next (Codex next turn, a later plan, a named skill) and what it waits on.
...
### Skills (`.agents/skills/`) — invoke with `$<name>`
...
### Review agents (`.codex/agents/`) — spawned by the skills, not user-invoked
