...
- Project rules (verbatim, every dispatch):
  - The six invariants from `AGENTS.md` "Invariants" — quote the ones the task touches, with the mechanism the consistency spec prescribes (freshness guard, dedup key, atomic update shape, ack after write)
  ...
  - Commit message: imperative, no Conventional Commits prefix, no assistant attribution — no `Co-Authored-By` footer and no mention of Codex or any AI tool
...
3. Follow `AGENTS.md` conventions and the project rules listed above.
