...
Opraveno 2026-09-12 podle `docs/plans/2026-09-12-review-fixes-plan.md` (10 commitů `01dd326..8658b8d`, 202 unit testů). Položky níže zůstávají hotové; opravy se týkaly redakce hesel v logu, hranic kontraktu, dekodéru rámců a konfigurace.
...
Hotovo 2026-09-13 podle `docs/specs/2026-09-13-ingest-design.md` a `docs/plans/2026-09-13-ingest-plan.md` (30 commitů `40b3cd5..`, 209 testů ingest, celkem 538). Skriptovaný běh proti RabbitMQ 4.3 prošel všemi šesti scénáři; výsledky jsou v hlavičce plánu.
...
Hotovo 2026-09-14 podle `docs/specs/2026-09-13-processing-design.md` a `docs/plans/2026-09-13-processing-plan.md` (41 commitů `4673305..`, 288 testů processing, celkem 855). Skriptovaný běh proti RabbitMQ 4.3 a MongoDB 8.0 prošel všemi dvanácti scénáři; výsledky jsou v hlavičce plánu.
...
Hotovo 2026-09-15 podle `docs/specs/2026-09-14-integration-tests-design.md` a `docs/plans/2026-09-15-integration-tests-plan.md` (26 commitů `4e31c30..`, 32 integračních testů ve 4 souborech nad skutečným RabbitMQ 4.3 a MongoDB 8.0 z `docker-compose.test.yml`, plus 39 unit testů harnessu (generátor zátěže, parser konfigurace Compose, zachytávání logů, orákulum koncového stavu, spuštění příkazu s časovým limitem); celkem 952 testů v 49 souborech). Testovací stack (projekt `telemetry-test`, porty 5673/15673/27018) spouští a odstraňuje `globalSetup` projektu `integration`; každý test má vlastní virtual host a databázi. Celá integrační sada trvala 68 s (měřeno 2026-09-15); `pnpm test:unit` běží bez Dockeru. CI: `.github/workflows/ci.yml` (GitHub Actions, ubuntu-latest) spouští format:check, lint, typecheck a `pnpm test` včetně integračních testů. Zelené od běhu 34976955158 na commitu f5b4fc6 (2026-09-15, po opravě C11b a dvou nestabilních unit testů; `docs/plans/2026-09-15-ci-followup-plan.md`). Oprava T70 (`547b310`): consumer při zastavení zavře kanál a počká na jeho close-ok před zavřením spojení. Doplněno 2026-09-15 po `/verify`: close-ok dokazuje jen to, že kanál potvrzení přijal, ne že je fronta použila — klient quorum fronty v RabbitMQ 4.3.5 odkládá potvrzení nad 32 čekajících příkazů a při zavření kanálu je už neodešle; na CI runneru (2 vCPU) se tak 16 z 50 zpráv doručilo znovu. Předání při graceful stop je tedy at-least-once: C11b tvrdí `b.received === 151 + n` (zbylých 150, jedna zpráva-bariéra a `n` znovu doručených) a každou znovu doručenou zprávu jako duplikát bez efektu (`duplicate`, `stale`); test nejprve počká, až zrušený consumer zastavené instance zmizí z mapy consumerů fronty (živý dotaz `checkQueue`: fronta ho drží, dokud vše, co držel, není potvrzeno nebo vráceno), a teprve pak pošle bariéru, která se doručí za vším, co fronta vrátila — zachytí tak i znovudoručení, které je ještě na cestě; T70 zůstává v seznamu kompromisů jako známý limit (`docs/plans/2026-09-15-ci-followup-plan.md`). Doplněno 2026-09-15 po review plánu: `test/integration/harness-recovery.test.ts` (H1–H7) dokazuje, že `dispose()` po testu, který selhal nebo vypršel s pauzou, zastavením či alarmem v platnosti, obnoví stack a odstraní vhost i databázi testu (ověřeno proti záměrně rozbitému `dispose()`); `docker compose up` a `down -v` v globálním setupu běží pod časovým limitem 600 s / 120 s (`runInherited`), po jehož vypršení je příkaz ukončen a setup selže s jeho jménem.
...
Kontrola 2026-09-15 nad `679ad42` (čistý clone z GitHubu; do `3195e83` se změnilo jen formátování plánu). `node scripts/compose-check.mjs` z clone bez `.env`: vše PASS. `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm typecheck` a `pnpm test` v clone prošly (953 testů ve 49 souborech). V repozitáři není sledovaný `.env`, `.DS_Store`, `node_modules`, `dist` ani klíč; shody vzorů pro hesla jsou jen testovací URL pro redakci logu. CI na `679ad42` padlo na `format:check` neformátovaného plánu; opraveno commitem `3195e83`. Revize historie (206 commitů) našla šest slabých předmětů; historie přepsána 2026-09-15 v noci (odstavec níže).

Doplněno 2026-09-15 večer: po `40c3bd5` přibyly čtyři commity — `6878bf7` (`StorePort.applyState` přijímá zprávu a `receivedAt`, sekce 5), `30c157e` (odstraněn neprovedený plán `docs/plans/2026-09-15-hexagonal-boundaries-plan.md`; provedena jen změna signatury), `2da617b` (úklid po vlastní revizi) a `b7b1670` (globální setup integračních testů vytiskne logy kontejnerů, když se stack nepodaří spustit). CI na `30c157e` (běh 34990221764) dvakrát padlo ještě před prvním testem: kontejner RabbitMQ skončil s kódem 1 pět sekund po startu a `down -v` ho odstranil dřív, než šlo přečíst jeho log; stejný image i Compose soubor prošly 40 minut předtím i 20 minut poté (běh 34993571993 na `b7b1670` zelený). Běh 34994316331 na `6af64bb` padl stejně a díky výpisu logů ukázal příčinu: `Error when reading /var/lib/rabbitmq/.erlang.cookie: eacces` — health check (`rabbitmq-diagnostics` přes `docker exec`, tedy jako root) se na pomalém runneru spustil dřív, než server soubor s Erlang cookie vytvořil, a založil ho jako root; server ho pak nemohl přečíst a skončil s kódem 1 (dokumentovaná „shared secret seeding race condition" v CLI průvodci RabbitMQ). Opraveno v obou Compose souborech: probe běží jako uživatel `rabbitmq` přes `gosu` (compose spec, rozhodnutí 10).

Přepis historie 2026-09-15 v noci. Šest předmětů přepsáno (hash před přepisem → po přepisu): `18e3e09` → `69a8bab` „record the .local convention" → „Record the .local convention"; `0f8a53f` → `4a842a1` „plan and design" → „Add the shared contract design spec and implementation plan"; `06e2387` → `548f8cb` „Implement code changes to enhance functionality and improve performance" → „Add the review fixes implementation plan"; `95f4d98` → `353506a` „feat: enhance envInt validation and introduce timer limits" → „Bound env integers and timers, reject invalid UTF-8, add settleWithin" (tělo zachováno); `3a3cf3a` → `f257340` „test: add health server tests for readiness and connection handling" → „Add health server tests for readiness and connection handling"; `ca306cd` → `679ad42` „Add hexagonal boundaries implementation plan and extract application-owned ports" → „Add the hexagonal boundaries implementation plan". Přepis (`git filter-branch --msg-filter`) změnil jen texty zpráv: stromy, autoři, časy i oba merge commity zůstaly beze změny (ověřeno po dvojicích starý/nový commit, 213 commitů, 0 rozdílů). Od prvního přepsaného commitu se mění všechny hashe, proto byly citace commitů v tělech pěti commitů a 126 citací v `TODO.md`, specech a plánech přemapovány na nové hashe (248 náhrad); čtyři citace commitů, které v historii nebyly už před přepisem (`46ffb4d`, `9c37791`, `a1f3736`, `d31f8a4`), zůstaly. Mapa starý → nový hash je v negitovaném `.local/history-rewrite-2026-09-15/commit-map.txt`. Publikuje se force pushem s `--force-with-lease`.

- [x] Čistý clone repozitáře → spuštění systému jedním příkazem → systém běží, testy projdou.
- [x] Projít commit historii: smysluplná, bez balastu, odpovídá standardní týmové práci.
