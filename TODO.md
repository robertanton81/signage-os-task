...
Hotovo 2026-09-15 podle `docs/specs/2026-09-14-integration-tests-design.md` a `docs/plans/2026-09-15-integration-tests-plan.md` (19 commitů `d5d858d..`, 25 integračních testů ve 3 souborech nad skutečným RabbitMQ 4.3 a MongoDB 8.0 z `docker-compose.test.yml`, plus 26 unit testů harnessu (generátor zátěže, parser konfigurace Compose, zachytávání logů, orákulum koncového stavu); celkem 931 testů). Testovací stack (projekt `telemetry-test`, porty 5673/15673/27018) spouští a odstraňuje `globalSetup` projektu `integration`; každý test má vlastní virtual host a databázi. Celá integrační sada trvala 68 s (měřeno 2026-09-15); `pnpm test:unit` běží bez Dockeru. CI: `.github/workflows/ci.yml` (GitHub Actions, ubuntu-latest) spouští format:check, lint, typecheck a `pnpm test` včetně integračních testů.
