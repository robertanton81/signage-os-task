...
Doplněno 2026-09-15 večer: po `6c309cd` přibyly čtyři commity — `bf9043e` (`StorePort.applyState` přijímá zprávu a `receivedAt`, sekce 5), `3576e43` (odstraněn neprovedený plán `docs/plans/2026-09-15-hexagonal-boundaries-plan.md`; provedena jen změna signatury), `b88b26e` (úklid po vlastní revizi) a `26e66a8` (globální setup integračních testů vytiskne logy kontejnerů, když se stack nepodaří spustit). CI na `3576e43` (běh 34990221764) dvakrát padlo ještě před prvním testem: kontejner RabbitMQ skončil s kódem 1 pět sekund po startu a `down -v` ho odstranil dřív, než šlo přečíst jeho log; stejný image i Compose soubor prošly 40 minut předtím i 20 minut poté (běh 34993571993 na `26e66a8` zelený). Běh 34994316331 na `b65e611` padl stejně a díky výpisu logů ukázal příčinu: `Error when reading /var/lib/rabbitmq/.erlang.cookie: eacces` — health check (`rabbitmq-diagnostics` přes `docker exec`, tedy jako root) se na pomalém runneru spustil dřív, než server soubor s Erlang cookie vytvořil, a založil ho jako root; server ho pak nemohl přečíst a skončil s kódem 1 (dokumentovaná „shared secret seeding race condition" v CLI průvodci RabbitMQ). Opraveno v obou Compose souborech: probe běží jako uživatel `rabbitmq` přes `gosu` (compose spec, rozhodnutí 10).
